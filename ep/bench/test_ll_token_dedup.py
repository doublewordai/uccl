"""Changing-route LL regression with exact expert counts and source-index checks.
...
def main():
    ...
    def roundtrip(xx=x, ii=ids, ww=weights):
        rx, counts, handle, event, _ = buffer.low_latency_dispatch(
            xx,
            ii,
            T,
            a.experts,
        ...
        if a.fp8:
            q, scale = rx
            decoded = (
                q.float().view(G, world * T, H // 128, 128)
                * scale.float().unsqueeze(-1)
            ).reshape(G, world * T, H)
        else:
            decoded = rx.float()
        supplied = (decoded * factors).to(torch.bfloat16)
        y, event, _ = buffer.low_latency_combine(
            supplied,
            ii,
            ww,
            handle,
        ...
        event.current_stream_wait()
        return y, counts, handle, rx, supplied

    worst = 0.0

    def check(result, trial, actual=T, actual_by_rank=None):
        nonlocal worst
        y, counts, handle, _, _ = result
        expected = (
            (
                x[:actual].float()[:, None, :]
        ...
        worst = max(worst, rel)
        assert rel < (0.045 if a.fp8 else 0.012), (rank, trial, rel)
        src, layout = handle[:2]
        src = src.cpu()
        layout = layout.cpu()
        counts = counts.cpu()
    ...
    if rank == 0:
        print(
            f"PASS 10 eager calls + {a.replays * 2} captured calls + true empty/ragged inputs, exact routing metadata, max global relL2 {worst:.6f}",
            flush=True,
        )
